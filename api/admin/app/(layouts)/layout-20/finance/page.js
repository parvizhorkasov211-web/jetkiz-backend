...
function FinancePage() {
    ...
    const [weatherEnabled, setWeatherEnabled] = (0, react_1.useState)(false);
    const [clientDeliveryFeeDefault, setClientDeliveryFeeDefault] = (0, react_1.useState)("");
    const [clientDeliveryFeeWeather, setClientDeliveryFeeWeather] = (0, react_1.useState)("");
    const [courierPayoutDefault, setCourierPayoutDefault] = (0, react_1.useState)("");
    const [courierPayoutWeather, setCourierPayoutWeather] = (0, react_1.useState)("");
    const load = async () => {
        try {
            setLoading(true);
            setError(null);
            const json = await (0, api_1.apiFetch)("/orders/finance/config");
    ...
    const save = async () => {
        try {
            ...
            await (0, api_1.apiFetch)("/orders/finance/config", {
                method: "PATCH",
    ...
    return (<div className="p-6">
      <div className="flex items-center justify-between gap-3 mb-6">
        ...
        <button className="px-4 py-2 rounded bg-black text-white disabled:opacity-60" onClick={save} disabled={saving}>
          {saving ? "Сохранение…" : "Сохранить"}
        </button>
      </div>

      {error && (<div className="mb-4 rounded border border-red-200 bg-red-50 p-3 text-sm text-red-700">
          {error}
        </div>)}
      {info && (<div className="mb-4 rounded border border-green-200 bg-green-50 p-3 text-sm text-green-700">
          {info}
        </div>)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded border p-4">
          ...
          <input className="mt-1 w-full border rounded px-3 py-2" value={clientDeliveryFeeDefault} onChange={(e) => setClientDeliveryFeeDefault(e.target.value)} placeholder="Напр. 1200"/>

          <div className="mt-4 text-sm text-gray-600">Выплата курьеру (обычно)</div>
          <input className="mt-1 w-full border rounded px-3 py-2" value={courierPayoutDefault} onChange={(e) => setCourierPayoutDefault(e.target.value)} placeholder="Напр. 1100"/>
        ...
        <div className="rounded border p-4">
          ...
          <input className="mt-1 w-full border rounded px-3 py-2" value={clientDeliveryFeeWeather} onChange={(e) => setClientDeliveryFeeWeather(e.target.value)} placeholder="Напр. 1500"/>

          <div className="mt-4 text-sm text-gray-600">Выплата курьеру (в режиме)</div>
          <input className="mt-1 w-full border rounded px-3 py-2" value={courierPayoutWeather} onChange={(e) => setCourierPayoutWeather(e.target.value)} placeholder="Напр. 1500"/>
