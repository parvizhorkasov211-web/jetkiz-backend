...
function OrderDetailsPage() {
    ...
    const [selectedCourierUserId, setSelectedCourierUserId] = (0, react_1.useState)("");
    const fullCourierName = (0, react_1.useMemo)(() => {
        if (!order?.courier)
            return "Не назначен";
    ...
    (0, react_1.useEffect)(() => {
        ...
        setErr(null);
        Promise.all([
            (0, api_1.apiFetch)(`/orders/${id}`),
            (0, api_1.apiFetch)(`/couriers?page=1&limit=300`),
        ])
            .then(([o, c]) => {
            if (!alive)
                return;
    ...
    async function refreshOrder() {
        const o = (await (0, api_1.apiFetch)(`/orders/${id}`));
        setOrder(o);
        setSelectedCourierUserId(o?.courierId || "");
    }
    async function assignCourier(courierUserId) {
        if (!id)
            return;
        setAssigning(true);
        setErr(null);
        try {
            await (0, api_1.apiFetch)(`/orders/${id}/assign-courier`, {
                method: "PATCH",
    ...
    async function autoAssign() {
        ...
        setErr(null);
        try {
            await (0, api_1.apiFetch)(`/orders/${id}/auto-assign`, { method: "PATCH" });
            await refreshOrder();
        }
        catch (e) {
            ...
            <div className="card-body">
              <div className="d-flex flex-wrap align-items-center justify-content-between gap-3">
                ...
                <div className="d-flex flex-wrap align-items-center gap-2">
                  <select className="form-select" style={{ width: 360 }} value={selectedCourierUserId} onChange={(e) => setSelectedCourierUserId(e.target.value)} disabled={assigning}>
                    <option value="">— выбрать курьера —</option>
                    {couriers.map((c) => {
                const label = `${c.isOnline ? "🟢" : "🔴"} ${c.firstName} ${c.lastName} (${c.user?.phone || "-"})`;
                return (<option key={c.userId} value={c.userId}>
                          {label}
                        </option>);
            })}
                  </select>

                  <button className="btn btn-primary" disabled={assigning || !selectedCourierUserId} onClick={() => assignCourier(selectedCourierUserId)} title="Назначить выбранного курьера">
                    {assigning ? "..." : "Назначить"}
                  </button>

                  <button className="btn btn-light" disabled={assigning} onClick={autoAssign} title="Автоматически выбрать онлайн-курьера">
                    {assigning ? "..." : "Автоназначить"}
                  </button>

                  <button className="btn btn-outline-danger" disabled={assigning || !order.courierId} onClick={unassign} title="Снять курьера с заказа">
                    Снять
                  </button>
                </div>
              </div>
            ...
            <div className="card-body">
              ...
              <div className="table-responsive">
                <table className="table table-striped align-middle">
                  ...
                  <tbody>
                    {(order.items || []).map((it) => (<tr key={it.id}>
                        <td>{it.title}</td>
                    ...
                    {!loading && (!order.items || order.items.length === 0) && (<tr>
