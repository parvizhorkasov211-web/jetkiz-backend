...
function rfmStatusRu(totalSpent: number, totalOrders: number, recencyDays: number | null) {
  if (totalOrders <= 0) return 'Новый';
  if (recencyDays == null) return 'Неизвестно';

  if (totalSpent >= 100000 && totalOrders >= 10 && recencyDays <= 30) return 'Топ-клиент';
  if (totalOrders >= 5 && recencyDays <= 45) return 'Постоянный';
  if (totalOrders >= 2 && recencyDays <= 30) return 'Перспективный';
  if (totalOrders >= 3 && recencyDays > 60) return 'Рискуем потерять';
  if (recencyDays > 45) return 'Спящий';
...
export class RestaurantMetricsService {
  ...
  async getRestaurantMetrics(input: {
    ...
    const deliveredRatePercent =
      totalOrders > 0 ? Math.round((deliveredCount / totalOrders) * 100) : 0;

    // daily график
    const dailyMap = new Map<
      string,
      { date: string; orders: number; delivered: number; canceled: number; revenue: number; paid: number }
    >();
    ...
    for (const o of orders) {
      ...
      if (o.status === 'CANCELED') cur.canceled += 1;
      if (o.paymentStatus === 'PAID') cur.paid += 1;
      if (o.status === 'DELIVERED' && o.paymentStatus === 'PAID') cur.revenue += o.total || 0;
...

    {
      ...
      while (cur.getTime() <= last.getTime()) {
        ...
        const row =
          dailyMap.get(key) || { date: key, orders: 0, delivered: 0, canceled: 0, paid: 0, revenue: 0 };
        daily.push(row);
    ...
    for (const o of orders) {
      ...
      if (diffDays <= 7) last7.add(o.userId);
      if (diffDays <= 30) last30.add(o.userId);
    }

    const userFirstInPeriod = new Map<string, Date>();
    for (const o of orders) {
      const prev = userFirstInPeriod.get(o.userId);
      if (!prev || o.createdAt.getTime() < prev.getTime()) userFirstInPeriod.set(o.userId, o.createdAt);
    }
    ...
    for (const o of orders) perUserCount.set(o.userId, (perUserCount.get(o.userId) || 0) + 1);
    ...
    const reviewRatePercent =
      deliveredCount > 0 ? Math.round((reviewsCount / deliveredCount) * 100) : 0;
    ...
    for (const o of orders) {
      ...
      u.ordersCount += 1;
      if (o.status === 'DELIVERED' && o.paymentStatus === 'PAID') u.spent += o.total || 0;
    ...
    // тренд выручки 30 vs предыдущие 30
    let trendRevenuePercent: number | null = null;
    {
      ...
      allowTrend: if (aFrom.getTime() >= since.getTime()) {
        ...
        if (sumB > 0) trendRevenuePercent = Math.round(((sumA - sumB) / sumB) * 100);
        else trendRevenuePercent = sumA > 0 ? 100 : 0;
      }
    }

    const suggestions: { type: 'warning' | 'info' | 'success'; title: string; text: string }[] = [];
    ...
    // ✅ последние заказы (для админки, кликабельная таблица)
